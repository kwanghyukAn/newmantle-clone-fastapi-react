...
export function SoloPage({ playerName }: Props) {
  ...
  return (
    <section className="worksheet-grid worksheet-grid-solo">
      <section className="worksheet-region worksheet-title">
        ...
        <p className="hero-copy">한국어 일반명사만 대상으로 의미 유사도를 계산합니다. 입력할수록 정답에 더 가까워집니다.</p>
      </section>
      ...
      <section className="worksheet-region worksheet-input">
        <form className="guess-form" onSubmit={handleSubmit}>
          <input
            value={word}
            onChange={(event) => setWord(event.target.value)}
            placeholder="정답일 것 같은 단어를 입력"
          />
          <button type="submit">추측</button>
        </form>
      </section>

      <section className="worksheet-region worksheet-actions">
      ...
      <section className="worksheet-region worksheet-status">
        {state?.completed ? (
      ...
      {embeddingInfo ? (
        <section className="worksheet-region worksheet-diagnostics">
          <div className="worksheet-region-head">
            <h3>환경 진단</h3>
